  ...
  Widget build(BuildContext context) {
    return Consumer<GroupViewModel>(
      builder: (context, gvm, child) {
        var index = 0;
        final List<Widget> devices = gvm.devices
            .map((dvm) => ChangeNotifierProvider.value(
                value: dvm, child: DeviceTile(index == gvm.devices.length - 1)))
            .toList();
        return Column(
            crossAxisAlignment: CrossAxisAlignment.stretch, children: devices);
      },
    );
  }
}
...
class DevicesScreen extends StatelessWidget {
  ...
  SliverAppBar _buildAppBar(BuildContext context) {
    return SliverAppBar(
      ...
      title: Text(
        context.translate('Devices in {currentScene}', nArgs: {
          'currentScene':
  ...
  Widget _buildGroupSection(BuildContext context, GroupViewModel g) {
    return ChangeNotifierProvider<GroupViewModel>.value(
      value: g,
      builder: (context, child) => Consumer<GroupViewModel>(
        builder: (context, gvm, child) {
                    ...
                    child: Row(children: [
                      ...
                      if (!gvm.isDummy)
                        IconButton(
                          ...
                          constraints: null,
                          onPressed: gvm.isDummy || gvm.isBusy
                              ? null
                              : () => _showEditGroupPage(context, g.model),
                        ),
                    ]),
