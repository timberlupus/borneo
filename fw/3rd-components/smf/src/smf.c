...
static const struct smf_state* get_last_of(const struct smf_state* states) { return get_child_of(states, NULL); }
...
void smf_set_state(struct smf_ctx* const ctx, const struct smf_state* new_state)
{
     ...
     */
    if (internal->is_exit) {
        ESP_LOGE(TAG, "Calling %s from exit action", __func__);
        return;
    }
