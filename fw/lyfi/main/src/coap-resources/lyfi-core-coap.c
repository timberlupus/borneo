                                     ...
                                     const coap_string_t* query, coap_pdu_t* response)
{
    ...
    BO_COAP_VERIFY(cbor_encode_float(&root_map, _led.settings.acclimation.start_utc));

    BO_COAP_VERIFY(cbor_encode_text_stringz(&root_map, "days"));
    BO_COAP_VERIFY(cbor_encode_float(&root_map, _led.settings.acclimation.duration));

    BO_COAP_VERIFY(cbor_encode_text_stringz(&root_map, "startPercent"));
    BO_COAP_VERIFY(cbor_encode_float(&root_map, _led.settings.acclimation.start_percent));
